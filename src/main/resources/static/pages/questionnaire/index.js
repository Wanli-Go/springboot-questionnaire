...
const fetchProjectList = () => {
  ...
  //alert(params.createdBy) // pops "undefined"
  $.ajax({
          ...
          <div class="list">
            <div class="list-header">
              ...
              <div>
                ...
                <button type="button" class="btn btn-link" onclick="onCheckStatistics('${item.id}')">统计</button>
              </div>
            </div>
            <div class="list-footer">
              <div>要查看调查问卷，请点击右上角的查看按钮。</div>
            </div>
          </div>
        `)
      })
...
const onCheckStatistics = (projectId) => {
  sessionStorage.setItem('currentProjectId', projectId);
  sessionStorage.setItem('projects', JSON.stringify(projectList))
  location.href = "/pages/projectStatistics/index.html"
}
...
const onCreateQuestionnaire = (projectId) => {
  sessionStorage.setItem('currentProjectId', projectId);
